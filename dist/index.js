...
// backend/src/comedian_do.ts
import { DurableObject } from "cloudflare:workers";
var ARCHETYPES = {
  deadpan_cynic: {
    name: "Deadpan Cynic",
    systemPrompt: "You are a deadpan, cynical standup comedian. Your style is dry, flat, and slow. You find the absurdity in modern technological decay and everyday nuisances. Avoid excitement, emojis, or exclamation points. Your humor comes from stating horrifying truths in a bored monotone.",
    rate: 0.88,
    pitch: 0.92
  },
  self_deprecating_neurotic: {
    name: "Self-Deprecating Neurotic",
    systemPrompt: "You are a highly neurotic, self-deprecating standup comedian. You overanalyze small social interactions and highlight your own failures with anxious energy. Your delivery is fast-paced, punctuated by rapid realizations and tangents that somehow circle back to make the punchline land harder.",
    rate: 1.12,
    pitch: 1.05
  },
  surrealist_storyteller: {
    name: "Surrealist Storyteller",
    systemPrompt: "You are a surrealist storyteller comedian. You take mundane situations like ordering coffee or buying groceries and escalate them into bizarre, dream-like scenarios with a completely straight face. The audience should not know whether you are serious until the punchline lands.",
    rate: 1,
    pitch: 1
  }
};
...
var ComedianDO = class extends DurableObject {
  static {
    __name(this, "ComedianDO");
  }
  state;
  env;
  ...
  async fetch(request) {
    ...
    const username = await this.state.storage.get("username") || "AI_Comic";
    ...
    if (url.pathname.endsWith("/schedule")) {
      ...
      if (currentAlarm === null) {
        await this.state.storage.setAlarm(Date.now() + 1e4);
      }
      return new Response(
  ...
  async alarm() {
    ...
    await this.state.storage.setAlarm(Date.now() + 2 * 60 * 60 * 1e3);
  }
  // ── Core Generation Pipeline ─────────────────────────────────────
  async generateJokeAndTTS(username) {
    const hourSeed = (/* @__PURE__ */ new Date()).getHours();
    let hash = 0;
    for (let i = 0; i < username.length; i++) {
      hash = username.charCodeAt(i) + ((hash << 5) - hash);
    }
    const archetypeKey = ARCHETYPE_KEYS[(Math.abs(hash) + hourSeed) % ARCHETYPE_KEYS.length];
...
2. NEVER use these clich\xE9 openings: "Have you ever noticed...", "So I was thinking...", "Why do they call it...", "What's the deal with..."
...
I installed a smart doorbell that recognizes faces. [PAUSE:1.0] Last night it sent me an alert: "Unrecognized person at the door." [PAUSE:1.5] It was me. Without coffee.`;
    ...
    const jokeText = aiResponse.response || "My smart fridge sent me a weekly screen time report. [PAUSE:1.5] Apparently, I spent 12 hours looking at cheese.";
      ...
      if (seg.type === "speech" && seg.text) {
        ...
        for (let j = 0; j < bareParts.length; j++) {
          ...
          if (j < bareParts.length - 1) {
            finalSegments.push({ type: "pause", durationMs: 1e3 });
          }
        }
      } else {
  ...
  const jokesWithHeckles = await Promise.all(
    (jokes || []).map(async (joke) => {
      ...
      return {
        ...
        has_audio: hasAudio,
        segments: joke.segments ? JSON.parse(joke.segments) : null,
        topHeckle
      };
    })
  );
...
app.get("/api/jokes/:id/audio", async (c) => {
  ...
  const result = await c.env.DB.prepare("SELECT audio_data FROM jokes WHERE id = ?").bind(id).first();
...
app.post("/api/tts", async (c) => {
  try {
    ...
    const cleanText = text.replace(/\[PAUSE\]/gi, " ").replace(/[#*$_[\](){}]/g, "").replace(/https?:\/\/\S+/gi, "").replace(/\s+/g, " ").trim();
...
app.get("/api/lineups/:id", async (c) => {
  const id = c.req.param("id");
  const lineup = await c.env.DB.prepare("SELECT * FROM lineups WHERE id = ?").bind(id).first();
...
app.post("/api/lineups", async (c) => {
  ...
  const id = crypto.randomUUID();
  await c.env.DB.prepare("INSERT INTO lineups (id, name, author_name) VALUES (?, ?, ?)").bind(id, name, authorName).run();
  for (let i = 0; i < jokeIds.length; i++) {
    await c.env.DB.prepare("INSERT INTO lineup_jokes (lineup_id, joke_id, position) VALUES (?, ?, ?)").bind(id, jokeIds[i], i).run();
  }
...
app.post("/api/rooms", async (c) => {
  ...
  ).bind(id, name, lineupId || null, (/* @__PURE__ */ new Date()).toISOString()).run();
  return c.json({ id, name, current_lineup_id: lineupId });
});
app.get("/api/rooms/:id/poll", async (c) => {
  const id = c.req.param("id");
  const room = await c.env.DB.prepare("SELECT * FROM club_rooms WHERE id = ?").bind(id).first();
  if (!room) return c.text("Room not found", 404);
  await c.env.DB.prepare("DELETE FROM room_reactions WHERE room_id = ? AND datetime(created_at) < datetime('now', '-10 seconds')").bind(id).run();
  ...
  return c.json({
    ...
    reactions: (reactions || []).map((r) => r.reaction_type)
  });
});
app.post("/api/rooms/:id/react", async (c) => {
  ...
  await c.env.DB.prepare("INSERT INTO room_reactions (id, room_id, reaction_type) VALUES (?, ?, ?)").bind(reactionId, id, reactionType).run();
  return c.json({ success: true });
});
app.post("/api/rooms/:id/next", async (c) => {
  ...
  ).bind(index, (/* @__PURE__ */ new Date()).toISOString(), id).run();
  return c.json({ success: true });
});
app.get("/api/comedians/:username", async (c) => {
  const username = c.req.param("username");
  let profile = await c.env.DB.prepare("SELECT * FROM comedians WHERE username = ?").bind(username).first();
  if (!profile) {
    await c.env.DB.prepare("INSERT INTO comedians (username, bio) VALUES (?, ?)").bind(username, "New Comedian on the Block").run();
    profile = { username, bio: "New Comedian on the Block" };
  }
  const followerCount = await c.env.DB.prepare("SELECT COUNT(*) as count FROM follows WHERE comedian_username = ?").bind(username).first();
  ...
  const mappedJokes = (jokes || []).map((j) => ({
    ...
    created_at: j.created_at,
    has_audio: j.audio_data ? true : false,
    segments: j.segments ? JSON.parse(j.segments) : null
  }));
  return c.json({
...
app.post("/api/comedians/:username/follow", async (c) => {
  ...
  try {
    await c.env.DB.prepare("INSERT INTO follows (follower_username, comedian_username) VALUES (?, ?)").bind(followerName, username).run();
  } catch (e) {
    if (e.message?.includes("UNIQUE constraint")) {
      await c.env.DB.prepare("DELETE FROM follows WHERE follower_username = ? AND comedian_username = ?").bind(followerName, username).run();
...
app.post("/api/club/generate-set", async (c) => {
  ...
  try {
    ...
    const content = response.response || "Thank you, you have been a great crowd!";
